...
interface DuvidaFormTemplateProps {
  ...
  duvidaId?: string | string[];
}

export default function DuvidaFormTemplate({ duvidaId }: DuvidaFormTemplateProps) {
  const router = useRouter();
  const isEditMode = !!duvidaId;
  ...
  const saveLabel = isEditMode ? 'Salvar edições' : 'Salvar dúvida';
  const perguntaLabel = isEditMode ? 'Edite a pergunta da dúvida' : 'Digite a pergunta da dúvida';
        ...
        >
          ...
          <View className="bg-transparent rounded-xl p-4 mb-6 border border-admin-border">
            ...
            <TextInput
              ...
              accessible={true}
              accessibilityLabel={perguntaLabel}
            />
          </View>
